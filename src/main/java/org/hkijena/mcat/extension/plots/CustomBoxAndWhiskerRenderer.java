...
package org.hkijena.mcat.extension.plots;
 ...
 */
public class CustomBoxAndWhiskerRenderer extends AbstractCategoryItemRenderer
    ...
    public void drawVerticalItem(Graphics2D g2, CategoryItemRendererState state,
            ...
            if (yMean != null) {
                ...
                if ((yyAverage > (dataArea.getMinY() - aRadius))
                    ...
                    Ellipse2D.Double avgEllipse = new Ellipse2D.Double(
                            xx + state.getBarWidth() / 2 - aRadius, yyAverage - aRadius, aRadius * 2,
                            aRadius * 2);
                    g2.fill(avgEllipse);
                    g2.draw(avgEllipse);
                }
        ...
        // draw outliers
        double oRadius = 3;    // outlier radius
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof CustomBoxAndWhiskerRenderer)) {
            return false;
        }
        CustomBoxAndWhiskerRenderer that = (CustomBoxAndWhiskerRenderer) obj;
        if (this.fillBox != that.getFillBox()) {
            return false;
        }
        if (this.itemMargin != that.getItemMargin()) {
            return false;
        }
        if (this.maximumBarWidth != that.getMaximumBarWidth()) {
            return false;
        }
        if (this.meanVisible != that.isMeanVisible()) {
            return false;
        }
        if (this.medianVisible != that.isMedianVisible()) {
            return false;
        }
        if (this.useOutlinePaintForWhiskers
